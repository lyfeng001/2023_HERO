...
#if USE_RMSTATE
		if(switch_is_mid(gimbal_control_set->gimbal_rc_ctrl->rc.s[0])){	//keyboard control 键鼠档
...
//			*pitch = -gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN;
		...
		else if(switch_is_up(gimbal_control_set->gimbal_rc_ctrl->rc.s[0])){	//remote control 遥控器档
